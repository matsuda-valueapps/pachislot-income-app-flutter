...
class DatabaseService {
  ...
  // 入力画面のSQLite導入時は1。
  // memo_recordsテーブル追加で2。
  // counter_recordsテーブル追加で3。
  ...
  /// すべての収支データを削除
  ///
  /// 将来的な「データ初期化」機能などで使用する。
  Future<int> deleteAllIncomeRecords()
      async {
    final db = await database;

    return db.delete(_incomeTable);
  }
  ...
  /// すべてのメモを削除
  ///
  /// 将来的な「メモデータ初期化」機能などで使用する。
  Future<int> deleteAllMemoRecords()
      async {
    final db = await database;

    return db.delete(_memoTable);
  }
  ...
  /// すべての小役カウンターを削除
  ///
  /// 将来的な「カウンターデータ初期化」機能などで使用する。
  Future<int> deleteAllCounterRecords()
      async {
    final db = await database;

    return db.delete(_counterTable);
  }
